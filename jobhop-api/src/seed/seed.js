...
await Connection.create({ userId: arash._id, contactUserId: sara._id, status: "ACCEPTED" });
await Connection.create({ userId: sara._id, contactUserId: arash._id, status: "ACCEPTED" });

// A pending request to Arash from Ali (so Arash can accept)
await Connection.create({ userId: ali._id, contactUserId: arash._id, status: "PENDING" });
...
console.log("Login with arash@example.com / password");
process.exit(0);